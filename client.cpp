...
Response LOGClient::UpdateACL(const std::string& project, const ACL& acl)
{
    return UpdateACL(project, "", acl);
}

Response LOGClient::UpdateACL(const std::string& project, const std::string& logStore, const ACL& acl)
{
    string body = acl.ToRequestJsonString();
    string operation = "/";
...
static void ExtractACLs(HttpMessage& httpMessage, vector<ACL>& aclsResult)
{
    rapidjson::Document document;
    try
    {
        cout << httpMessage.content << endl;
        ExtractJsonResult(httpMessage.content, document);
        
        const rapidjson::Value& acls = GetJsonValue(document, "acls");
        for (rapidjson::Value::ConstValueIterator itr = acls.Begin(); itr != acls.End(); ++itr)
        {
            if (itr->IsObject())
            {
                aclsResult.push_back(ACL());
                aclsResult.back().FromJson(*itr);
            }
            else
            {
                throw LOGException(LOGE_BAD_RESPONSE, string("Invalid json format, value is not string type\tbad json format:") + httpMessage.content, httpMessage.header[X_LOG_REQUEST_ID], httpMessage.statusCode);
...
ListACLsResponse LOGClient::ListACLs(const std::string& project, const int32_t& offset, const int32_t& size)
{
    return ListACLs(project, "", offset, size);
}

ListACLsResponse LOGClient::ListACLs(const std::string& project, const std::string& logStore, const int32_t& offset, const int32_t& size)
{ 
    string body = "";
    ...
    ListACLsResponse ret;
    ret.statusCode = httpResponse.statusCode;
    ret.requestId = httpResponse.header[X_LOG_REQUEST_ID];

    ExtractACLs(httpResponse, ret.result);
